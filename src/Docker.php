...
class Docker extends Client
{
     ...
     * @return Docker
     */
    public static function create($httpClient = null, array $additionalPlugins = []): Docker
    {
        if (!$httpClient) {
            $httpClient = HttpClientFactory::createFromEnv();
        }
    ...
    public function imagePush(
        ...
        if (isset($headerParameters['X-Registry-Auth']) && $headerParameters['X-Registry-Auth'] instanceof AuthConfig) {
            $headerParameters['X-Registry-Auth'] = base64_encode(
                $this->serializer->serialize($headerParameters['X-Registry-Auth'], 'json')
            );
        }
